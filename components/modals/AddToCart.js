...
const AddToCartModal = ({ addItem, setAddItem, setOpen, fetchCartData }) => {
    const [btnLoader, setBtnLoader] = useState(false)
    const [itemQty , setItemQty]= useState({})
    const handleClose = () => { setAddItem({ ...addItem, open: false }) };


    const handleQtyInc = (it) => {
        console.log("it",it)
        setItemQty(prev => ({
            ...prev,
            [addItem?.data?._id]: {
    ...
    // console.log("qty data", itemQty)

    const addToCarthandler = async () => {
        setBtnLoader(true)
        const dataToSend = { ...addItem.data, uid: JSON.parse(localStorage.getItem('UID')) }
    ...
    // console.log("addItem?.data",addItem?.data?.sizeList)
    ...
    return (
        <>
            <Modal open={addItem.open} onClose={handleClose} sx={{ border: "none" }} disableAutoFocus>
                {/* <Modal open={true} onClose={handleClose} sx={{border:"none"}} disableAutoFocus>   */}
                        ...
                        <Box sx={{ width: { lg: 220, md: 220, sm: 220, xs: 200 }, height: "100%", bgcolor: "#b2ebf2", display: "flex", alignItems: "center", justifyContent: "center" }}>
                            <Box sx={{ height: "100%", position: "relative",width:"80%",p:"0px 10px",mt:"5px" }}>
                                ...
                                <Box sx={{ display: "flex",justifyContent: "center",width:"100%", alignItems: "Center", mt: { lg: "15px", md: "15px", sm: "13px", xs: "8px" } }}>
                                    <Box sx={{ width:"100%"}}>
                                        {
                                            addItem?.data?.sizeList?.map((item) => {
